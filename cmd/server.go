...
// because HTTP/3 (which requires QUIC/TLS) is configured, an explicit
// listener asks for ssl or h3, global TLS cert paths are set, or any target
// declares per-target TLS cert paths. When a self-signed certificate is
// generated in this case, all HTTP versions (HTTP/1.1, HTTP/2, and HTTP/3)
// work out of the box.
...
func (ps *proxyServer) reload() error {
	return ps.reloadFrom("config.yaml", "config.d")
}
...
// listenerSpecs resolves the frontend listeners for start(). Explicit
// `listeners:` entries take precedence; otherwise the legacy `listen`
// shorthand is expanded, where TLS is implied by configured certificates
// and HTTP/3 runs on the same port (or on http3_addr when set).
func (ps *proxyServer) listenerSpecs() ([]config.ListenersSpec, error) {
	...
	listenAddr := ps.proxyCfg.Listen
	if listenAddr == "" {
		listenAddr = ":8081"
	} else if idx := strings.Index(listenAddr, ","); idx != -1 {
		listenAddr = strings.TrimSpace(listenAddr[:idx])
	}
	spec := config.ListenersSpec{Addr: listenAddr}
	if ps.tlsRequested() {
		spec.SSL = true
		if ps.proxyCfg.HTTP3Addr != "" {
			return []config.ListenersSpec{
				spec,
				{Addr: ps.proxyCfg.HTTP3Addr, QUIC: true},
			}, nil
		}
		spec.QUIC = true
	}
	return []config.ListenersSpec{spec}, nil
}
...
// startHTTP3 listens on the configured UDP address and serves HTTP/3 (QUIC)
// alongside the existing TCP frontend. It is a no-op when http3_addr is empty.
func (ps *proxyServer) startHTTP3(tlsConfig *tls.Config) error {
	var addr string
	if ps.proxyCfg != nil {
		addr = ps.proxyCfg.HTTP3Addr
	}
	if err := ps.startQuicListener(addr, tlsConfig); err != nil {
		return err
	}
	if len(ps.h3Conns) == 0 {
		return nil
	...
	if ps.h3Srv == nil {
		ps.h3Srv = ps.h3Srvs[len(ps.h3Srvs)-1]
		ps.h3Conn = ps.h3Conns[len(ps.h3Conns)-1]
	}
	h3Srv, conn := ps.h3Srvs[len(ps.h3Srvs)-1], ps.h3Conns[len(ps.h3Conns)-1]
	go func() {
		if err := h3Serve(h3Srv, conn); err != nil && !errors.Is(err, http.ErrServerClosed) {
			klog.Errorf("HTTP/3 server error: %v", err)
		}
	}()
	return nil
}
